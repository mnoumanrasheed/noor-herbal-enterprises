...
import { DEFAULT_BRAND_STORY_CONTENT, DEFAULT_CONTACT_SETTINGS, DEFAULT_HERO_SETTINGS } from "@/lib/site-settings";
...
async function getEditableSettings(): Promise<EditableSettings> {
  const rows = await sql`SELECT key, value FROM site_settings WHERE key IN ('contact_email', 'contact_phone', 'contact_instagram', 'hero_eyebrow', 'hero_title', 'hero_description', 'hero_cta_label', 'hero_visual_category', 'homepage_copy', 'featured_content', 'footer_content', 'story_heading', 'story_body_one', 'story_body_two', 'story_mission', 'story_vision', 'story_opening_image', 'story_opening_image_alt', 'story_opening_image_public_id', 'story_mission_image', 'story_mission_image_alt', 'story_mission_image_public_id', 'story_vision_image', 'story_vision_image_alt', 'story_vision_image_public_id')`;
